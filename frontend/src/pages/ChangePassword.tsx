...
import { Lock, ArrowRight, Eye, EyeOff, ShieldCheck, Cloud } from 'lucide-react';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (user) {
        ...
        const storage = localStorage.getItem('medos_user') ? localStorage : sessionStorage;
        storage.setItem('medos_user', JSON.stringify(user));
      }

      navigate('/');
    } catch (err: any) {
      setError(err.response?.data?.error || err.response?.data?.detail || 'Failed to change password. Please verify your old password.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <>
      ...
      <div className="login-page">
        ...
        <div className="login-content" style={{ marginTop: '-40px' }}>
          ...
          <h1 style={{ fontSize: '1.25rem', fontWeight: 600, color: '#0A6253', marginBottom: '4px' }}>
            Security Update Required
          </h1>
          <p className="tagline" style={{ marginBottom: '24px' }}>
          ...
          <form className="login-form" onSubmit={handleSubmit}>
            ...
            {/* Old Password */}
            <div>
              <label htmlFor="old-password" className="field-label">Old Password</label>
